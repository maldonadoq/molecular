...
class TNeighbor: public TMsa{
private:
	std::vector<std::vector<float> > m_distances;
	std::vector<std::vector<float> > m_q;
	int idx_center;
public:
	TNeighbor(int _match, int _mismatch, int _gap)
...
float TNeighbor::FPairwiseDistance(std::string _a, std::string _b){
	float ngap = 0;
	float ndif = 0;
...
void TNeighbor::FPrintDistances(){
	unsigned i,j;
